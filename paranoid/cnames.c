 ...
 **  @file cnames.c
 **  @author Hugh S. Myers
 **  @brief A collection of functions in aid of Rgb to color name and back.
 **  @date Mon Jul 24 17:25:46 2017
 **
 **  Give an Rgb color name retrieve the value. Given the value, retrieve the name.
 **  This allows color specification in human readable form regardless of which
 **  way the information is flowing.
 **
 **  @note gcc -Wall -DCNAMES_TEST -o nam cnames.c
 **
 **  @see https://en.wikipedia.org/wiki/Lists_of_colors
 **  @see https://github.com/codebrainz/color-names
 **
 **  @bug No known bugs.
 **/
...
int main( int argc, char *argv[] ) {
    ...
    if (argv[1] ) {
        printf( "First char = '%c'\n", argv[1][0] );
        if ( argv[1][0] == '{' ) {
            sscanf( argv[1], "{%hhu,%hhu,%hhu}", &color.r, &color.g, &color.b );
            printf( "color from '%s' = { %d, %d, %d }\n", argv[1], color.r, color.g, color.b );
            printf( "color name is: '%s'\n", RgbName( color ) );
        } else {
            if ( str2Rgb( argv[1], &color ) ) {
    ...
    char test[] = "SET:128";
    int i = atoi( test + strcspn( test, "1234567890" ) );
    printf("i = '%d'\n", i );
    char buf[128];
    ...
    memset(buf, 0, 128);
    strftime(buf, 128, "The timezone is '%Z'", ptm);
    puts(buf);
    char tz1[128];
    char tz2[128];

    str2abbr( tz1, sizeof(tz1), tzname[0] );
    str2abbr( tz2, sizeof(tz2), tzname[1] );
    printf( "tzname[0] = '%s(%s)', tzname[1] = '%s(%s)'\n", tzname[0], tz1, tzname[1], tz2 );
...
char *RgbName( Rgb color ) {
    ...
    for (int i = 0; i < COLOR_NAMES_MAX; ++i ) {
        if ( RgbEQ( color, color_data[i].rgb ) ) {
            return (char *)color_data[i].name;
        }
    }
    return NULL;
}
